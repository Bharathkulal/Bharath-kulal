# Hi, I’m Bharath 👋
### BCA AI & ML Student | Web Developer | AI Enthusiast

🌟 Passionate about building practical, feature-rich web apps with AI/ML integration.  
💻 Skilled in: HTML, CSS, JavaScript, Node.js, MongoDB, Next.js, TypeScript, TailwindCSS.  
🚀 Interests: Smart Campus Companion | Gym Galaxy | Creative Dashboards | Multilingual Features.  

---

## 🔧 Technologies & Tools
![HTML5](https://img.shields.io/badge/-HTML5-orange?logo=html5)
![CSS3](https://img.shields.io/badge/-CSS3-blue?logo=css3)
![JavaScript](https://img.shields.io/badge/-JavaScript-yellow?logo=javascript)
![Node.js](https://img.shields.io/badge/-Node.js-green?logo=node.js)
![MongoDB](https://img.shields.io/badge/-MongoDB-darkgreen?logo=mongodb)
![Next.js](https://img.shields.io/badge/-Next.js-black?logo=next.js)
![TypeScript](https://img.shields.io/badge/-TypeScript-blue?logo=typescript)
![TailwindCSS](https://img.shields.io/badge/-TailwindCSS-teal?logo=tailwindcss)

---

## 🚀 Projects
### 📚 Smart Campus Companion
A full-stack web app for students with AI-powered study tools, smart scheduling, community hub, and multilingual support.  
**Tech Stack:** Next.js, Node.js, MongoDB, TailwindCSS, TypeScript, AI/ML (NLP).

### 🏋️ Gym Galaxy
A fitness tracking and community app with modern dashboards, glowing UI, and personalized features.  
**Tech Stack:** Next.js, Node.js, MongoDB, TailwindCSS.

---


## 📊 GitHub Stats
![Bharath’s GitHub stats](https://github-readme-stats.vercel.app/api?username=Bharathkulal&show_icons=true&theme=radical)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Bharathkulal&layout=compact&theme=radical)
![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Bharathkulal&theme=radical)

## 📫 Connect With Me
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-blue?logo=linkedin)](YOUR_LINKEDIN_URL)  
[![Email](https://img.shields.io/badge/-Email-red?logo=gmail)](mailto:YOUR_EMAIL)

---

✨ *“Code with creativity, build with passion, and learn with curiosity.”*
